		...
		<%@ taglib prefix="fn" uri="http://java.sun.com/jsp/jstl/functions"%>
			<%@ taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt"%>
				...
				<head>
					...
					<script type="text/javascript">
						...
						$(document).ready(function() {
							...
							// dragover : Drag 중 Mouse 가 현재 위치한 Element
							win.on('dragover', function(e) {
								// drageover, dragenter은 되지만 drop이 안되므로
					...
					<div class="wrap">
						...
						<div class="box-body">
							<div class="form-group">
								<label>Title</label> <input type="text" name='title' class="form-control" value="${boardDTO.title}" readonly="readonly">
							</div>
							<div class="form-group">
								<label>Content</label>
							...
							<div class="form-group">
								<label>Writer</label> <input type="text" name="writer" class="form-control" value="${boardDTO.writer}" readonly="readonly">
							</div>
						</div>
						<!-- class="box-body" 끝 -->
				...
				LIST</button>
						...
						${fn:length(boardDTO.replyList)} ] </small>
							...
							<c:forEach items="${boardDTO.replyList}" var="replyDTO">
								<li class="time_sub" id="${replyDTO.rno}">
									...
									<p>
										<fmt:formatDate pattern="yyyy/MM/dd" dateStyle="short" value="${replyDTO.regdate}" />
									</p>
									<c:if test="${replyDTO.rupload!=null }">
										<p>
											<c:set var="numload" value="${fn:indexOf(replyDTO.rupload, '_') +1 }" />
											<c:set var="strlength" value="${fn:length(replyDTO.rupload)}" /> ${fn:substring(replyDTO.rupload,numload,strlength)}
										</p>
									</c:if>
					...
					<div id="modifyModal">
						<p>
							<label for="updateReplyText">Reply Text</label> <input class="form-control" type="text" placeholder="REPLY TEXT" id="updateReplyText">
						</p>
						<p>
